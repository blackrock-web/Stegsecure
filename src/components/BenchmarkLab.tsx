...
export const BenchmarkLab: React.FC = () => {
  ...
  const selectProposedOnly = () => {
    setSelectedModelIds(['proposed_lf_rinn_opm_epp_cnn']);
  };
  ...
  const bestResults: BestResultSummary = useMemo(() => {
    ...
    // 3. Best Overall (Averaged PSNR & SSIM across evaluated images per model)
    const modelAggregates = new Map<
      string,
      { modelName: string; count: number; totalPsnr: number; totalSsim: number }
    >();
        ...
        {bestResults.insufficientData ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-purple-50/50 border border-purple-200 rounded-xl">
              ...
              <p className="text-[11px] text-slate-400 mt-0.5">
                Composite Harmonic Score: {bestResults.bestOverall?.score}
              </p>
            </div>
          </div>
        )}
