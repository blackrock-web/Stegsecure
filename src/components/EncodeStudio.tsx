    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2 overflow-x-auto">
          <span className="text-xs text-slate-400 whitespace-nowrap">Sample Covers:</span>
          {SAMPLE_COVERS.map((sample) => (
            <button
              key={sample.id}
              onClick={() => loadSampleCover(sample)}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-medium text-slate-300 rounded-lg whitespace-nowrap transition-colors"
            >
              {sample.name.split(' ')[0]}
            </button>
          ))}
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
            ...
            <div
              ...
              className="border-2 border-dashed border-slate-700 hover:border-indigo-500 rounded-xl p-4 text-center cursor-pointer transition-colors bg-slate-950/40 relative group"
            >
              <input
                type="file"
                ref={fileInputRef}
              ...
              {previewUrl ? (
                <div className="flex flex-col items-center">
                  ...
                  />
                  <p className="text-xs text-slate-400 mt-2 font-mono truncate max-w-xs">
                    {selectedFile?.name} ({(selectedFile?.size ? (selectedFile.size / 1024).toFixed(1) : 0)} KB)
                  </p>
                  <span className="text-xs text-indigo-400 group-hover:underline mt-1">
                ...
                <div className="py-6 flex flex-col items-center">
                  ...
                  <p className="text-xs text-slate-300 font-medium">Drag & drop PNG cover image here</p>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm space-y-4">
            <div>
              ...
              <textarea
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 font-mono"
              />
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span className="text-xs text-emerald-400 font-mono">PBKDF2 10k iter</span>
              </div>
              <input
                type="text"
                value={passphrase}
              ...
              <div className="pt-2 space-y-3 border-t border-slate-800 text-xs">
                <div>
                  <div className="flex justify-between text-slate-300 mb-1">
                    ...
                    <span className="font-mono text-emerald-400">{(config.threshA * 100).toFixed(0)}%</span>
                  </div>
                  <input
                    type="range"
                    min="0.1"
                ...
                <div>
                  <div className="flex justify-between text-slate-300 mb-1">
                    ...
                    <span className="font-mono text-indigo-400">{(config.threshB * 100).toFixed(0)}%</span>
                  </div>
                  <input
                    type="range"
                    min="0.5"
                ...
                <div>
                  <div className="flex justify-between text-slate-300 mb-1">
                    ...
                    <span className="font-mono text-purple-400">{config.gamma.toFixed(2)}</span>
                  </div>
                  <input
                    type="range"
                    min="0.3"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-slate-400 block mb-1">Zone B OPAP bits (k_b):</label>
                    <select
                      value={config.kbBits}
                  ...
                  <div>
                    <label className="text-slate-400 block mb-1">Zone C OPAP bits (k_c):</label>
                    <select
                      value={config.kcBits}
          ...
          {/* Action Button & Error */}
          {error && (
            <div className="p-3 bg-red-950/60 border border-red-800/80 rounded-xl text-xs text-red-200 flex items-start space-x-2">
          ...
          <button
            ...
            className="w-full py-3 px-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 disabled:opacity-50 text-white font-bold text-sm rounded-xl shadow-lg transition-all flex items-center justify-center space-x-2 cursor-pointer disabled:cursor-not-allowed"
          >
            {isEncoding ? (
              <>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 shadow-sm">
            <div className="flex items-center justify-between mb-3">
              ...
              {isCalculatingCap ? (
                ...
                <span className="text-xs font-mono text-slate-300">
                  Max Payload: <strong className="text-emerald-400">{capacity?.maxBytes.toLocaleString()} B</strong> (~{capacity?.maxCharsEstimated.toLocaleString()} chars)
                </span>
              )}
            </div>
            ...
            <div className="space-y-1.5 mb-4">
              <div className="flex justify-between text-xs text-slate-400">
                ...
                <span className="font-mono text-indigo-300">{capacityUsedPct.toFixed(1)}%</span>
              </div>
              <div className="w-full bg-slate-950 rounded-full h-2.5 overflow-hidden border border-slate-800">
                <div
                  className={`h-full transition-all duration-300 ${
                    capacityUsedPct > 90 ? 'bg-red-500' : capacityUsedPct > 60 ? 'bg-yellow-500' : 'bg-emerald-500'
                  }`}
            ...
            <div className="grid grid-cols-3 gap-2 text-xs font-mono">
              <div className="bg-slate-950 p-2.5 rounded-lg border border-emerald-900/40">
                <span className="text-emerald-400 font-semibold block text-[11px]">Zone A (EMD)</span>
                <span className="text-slate-200 font-bold text-sm">{capacity?.zoneDistribution.zoneA}%</span>
                <span className="text-slate-400 block text-[10px]">{capacity?.zoneABytes} B</span>
              </div>

              <div className="bg-slate-950 p-2.5 rounded-lg border border-indigo-900/40">
                <span className="text-indigo-400 font-semibold block text-[11px]">Zone B (OPAP {config.kbBits}b)</span>
                <span className="text-slate-200 font-bold text-sm">{capacity?.zoneDistribution.zoneB}%</span>
                <span className="text-slate-400 block text-[10px]">{capacity?.zoneBBytes} B</span>
              </div>

              <div className="bg-slate-950 p-2.5 rounded-lg border border-amber-900/40">
                <span className="text-amber-400 font-semibold block text-[11px]">Zone C (OPAP {config.kcBits}b)</span>
                <span className="text-slate-200 font-bold text-sm">{capacity?.zoneDistribution.zoneC}%</span>
                <span className="text-slate-400 block text-[10px]">{capacity?.zoneCBytes} B</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div className="bg-slate-900 border border-slate-800 p-3 rounded-xl">
                  <span className="text-[11px] text-slate-400 block">PSNR Quality</span>
                  <span className="text-lg font-bold text-emerald-400 font-mono">
                    {encodeResult.metrics.psnrDb} dB
                  </span>
                  <span className="text-[10px] text-slate-500 block">(&gt;50dB is invisible)</span>
                </div>

                <div className="bg-slate-900 border border-slate-800 p-3 rounded-xl">
                  <span className="text-[11px] text-slate-400 block">SSIM Structural</span>
                  <span className="text-lg font-bold text-indigo-400 font-mono">
                    {encodeResult.metrics.ssim}
                  </span>
                  <span className="text-[10px] text-slate-500 block">(1.0 = identical)</span>
                </div>

                <div className="bg-slate-900 border border-slate-800 p-3 rounded-xl">
                  <span className="text-[11px] text-slate-400 block">Embedded Payload</span>
                  <span className="text-lg font-bold text-purple-400 font-mono">
                    {encodeResult.metrics.payloadBytes} B
                  </span>
                  <span className="text-[10px] text-slate-500 block">({encodeResult.metrics.bppEmbedded} bpp)</span>
                </div>

                <div className="bg-slate-900 border border-slate-800 p-3 rounded-xl">
                  <span className="text-[11px] text-slate-400 block">Security Verdict</span>
                  <span className="text-sm font-bold text-emerald-400 block truncate">
                    {encodeResult.securityReport.verdict}
                  </span>
                  <span className="text-[10px] text-slate-500 block">Risk: {encodeResult.securityReport.compositeRiskScore}/100</span>
                </div>
              </div>

              {/* Steganalysis Security Checks */}
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 text-xs space-y-2">
                <h3 className="font-bold text-slate-200 uppercase tracking-wider flex items-center space-x-1.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 font-mono pt-1">
                  <div className="bg-slate-950 p-2 rounded border border-slate-800">
                    <span className="text-slate-400 block text-[10px]">RS Steganalysis</span>
                    <span className="text-emerald-400 font-bold">
                      Est. {encodeResult.securityReport.rsAnalysis.estimatedEmbeddingRate}
                    </span>
                    <span className="text-[10px] text-slate-500 block">Status: {encodeResult.securityReport.rsAnalysis.status}</span>
                  </div>

                  <div className="bg-slate-950 p-2 rounded border border-slate-800">
                    <span className="text-slate-400 block text-[10px]">Chi-Square ($\chi^2$)</span>
                    <span className="text-indigo-400 font-bold">
                      p={encodeResult.securityReport.chiSquare.pValue}
                    </span>
                    <span className="text-[10px] text-slate-500 block">Status: {encodeResult.securityReport.chiSquare.pStatus}</span>
                  </div>

                  <div className="bg-slate-950 p-2 rounded border border-slate-800">
                    <span className="text-slate-400 block text-[10px]">Surrogate CNN Score</span>
                    <span className="text-purple-400 font-bold">
                      {(encodeResult.securityReport.surrogateCnnScore * 100).toFixed(1)}% prob
                    </span>
                    <span className="text-[10px] text-slate-500 block">Model: SteganalyzerNet</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2">
                <button
                  ...
                  className="flex-1 py-2.5 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow transition-colors flex items-center justify-center space-x-2"
                >
                ...
                <button
                  ...
                  className="py-2.5 px-4 bg-slate-800 hover:bg-slate-700 text-slate-200 font-medium text-xs rounded-xl border border-slate-700 transition-colors flex items-center space-x-1.5"
                >
                  <FileText className="w-4 h-4 text-indigo-400" />
